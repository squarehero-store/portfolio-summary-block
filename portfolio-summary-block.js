// 🚀 Portfolio Summary Block Adapter for Squarespace by SquareHero.store
(function() {
    // Add CSS for immediate hiding and loading indicator
    const style = document.createElement('style');
    style.textContent = `
      .sqs-block-summary-v2 .sqs-block-is-placeholder {
        visibility: hidden !important;
        opacity: 0 !important;
      }
      
      .portfolio-adapter-loading {
        position: relative;
        min-height: 200px;
        opacity: 0;
      ...
      .portfolio-adapter-loading-spinner {
        position: absolute;
        top: 50%;
        left: 50%;
        ...
        animation: portfolio-adapter-spin 1s ease-in-out infinite;
        z-index: 100;
      }
      
      @keyframes portfolio-adapter-spin {
        to { transform: translate(-50%, -50%) rotate(360deg); }
      }
    `;
    document.head.appendChild(style);
    
    // Execute hiding of placeholders immediately
    ...
    hideAllPlaceholders();
    setTimeout(hideAllPlaceholders, 1);
    ...
    document.addEventListener('DOMContentLoaded', function() {
          ...
          try {
            ...
            fetchPortfolioItems()
              .then(items => {
                ...
                // Remove spinner and show content with fade in
                setTimeout(() => {
                  ...
                  // Fix layout issues by triggering resize event after render
                  fixLayoutAfterRender(wrapper, blockConfig);
                  
                  // Log success message
                  console.log('🚀 SquareHero.store Portfolio Summary Block plugin by SquareHero.store loaded');
                }, 200);
              })
              .catch(error => {
                console.error('Error fetching portfolio items:', error);
                
                // Still remove spinner and show content even on error
                if (spinner && spinner.parentNode) {
                  spinner.parentNode.removeChild(spinner);
                }
                wrapper.style.visibility = 'visible';
      ...
      // Function to completely fix carousel navigation with improved resize handling
      ...
      // Initialize on page load with short delay to ensure DOM is fully built
      setTimeout(initPortfolioSummaryBlocks, 100);
      
      // Add a second trigger that runs slightly later to catch any delayed initializations
